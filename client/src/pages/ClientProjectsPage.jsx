...
const ClientProjectsPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                // Fetch all gigs and all reviews at the same time
                const [gigsRes, reviewsRes] = await Promise.all([
                    api.get('/gigs/my-gigs'),
                    api.get('/reviews/me/my-reviews')
                ]);

                // Filter for completed gigs
                setCompletedGigs(gigsRes.data.filter(gig => gig.status === 'Completed'));

                // Create a map of reviews for easy lookup
                const rMap = new Map(reviewsRes.data.map(review => [review.gig, review]));
                setReviewMap(rMap);
    ...
    if (loading) {
        return <div className="text-center mt-10">Loading your projects...</div>;
    }
                        ...
                        return (
                            <div key={gig._id} className="bg-white p-6 rounded-lg shadow-md">
                                ...
                                {review ? (
                                    ...
                                    <p className="text-gray-500 italic">You have not left a review for this project yet.</p>
                                )}
                            </div>
                        );
                    })}
